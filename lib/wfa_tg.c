...
int wfaSendBurstData(int mySockfd, int streamId, BYTE *pRespBuf, int *pRespLen)
{
    tgProfile_t           *theProf = findTGProfile(streamId);
    tgStream_t            *myStream =findStreamProfile(streamId);
    int                   ret = WFA_SUCCESS;
    ...
    DPRINT_INFO(WFA_OUT, "wfaSendBurstData entering\n");
    /* error check section  */
    if ( (mySockfd < 0) || (streamId < 0) || ( pRespBuf == NULL) 
            || ( pRespLen == NULL) )
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBurstData pass-in parameter err mySockfd=%i streamId=%i pRespBuf=0x%x pRespLen=0x%x\n",
            mySockfd,streamId,pRespBuf,pRespLen );
        ret= WFA_FAILURE;
        goto errcleanup;
    }
    ...
    if ( theProf == NULL || myStream == NULL)
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBurstData parameter err in NULL pt theProf=%l myStream=%l \n",
            theProf, myStream);
        ret= WFA_FAILURE;
        goto errcleanup;
    }
    if (theProf->burstperiodicity == 0 || theProf->duration == 0)
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBurstData usage error, burstPeriod=%i or duration=%i \n" , 
            theProf->burstperiodicity, theProf->duration);
        ret= WFA_FAILURE;
        goto errcleanup;
    }

    /* calculate bitrate asked */
    if ( (rate = theProf->pksize * theProf->burstfrags * (1000 / theProf->burstperiodicity) * 8) > WFA_SEND_FIX_BITRATE_MAX)
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBurstData over birate can do in the routine, req bitrate=%l \n",rate);
        ret= WFA_FAILURE;
        goto errcleanup;
    }
    /* alloc sending buff  */
    ...
    if ( packBuf == NULL)
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBurstData malloc err \n");
        ret= WFA_FAILURE;
        goto errcleanup;
    }
    memset(packBuf, 0, packLen + 4);
    ...
    while (runLoop)
    {
        gettimeofday(&before, NULL);
        /* send data per second loop */
        for ( i=0; i < (theProf->burstfrags); i++)
        {
           counter++;
           /* fill in the counter */
           int2BuffBigEndian(counter, &((tgHeader_t *)packBuf)->hdr[8]);
           ...
           */
           ...
           else
           {
               ...
               sleepTotal = sleepTotal + (unsigned long long int) 1;
               DPRINT_INFO(WFA_OUT, "wfaSendBurstData wfaTrafficSendTo call ERR counter=%i i=%i; send busy, sleep 1 MilSec then send\n", counter, i);
               i--;
           }
        ...
        gettimeofday(&after, NULL);
        difftime = wfa_itime_diff(&before, &after);
        if ( difftime < 0 || difftime >= theProf->burstperiodicity*1000)
        {/* over time used, no sleep, go back to send */
            nOverTimeCount++;
            if ( difftime > theProf->burstperiodicity*1000)
                extraTimeSpendOnSending += (difftime - theProf->burstperiodicity*1000);
            wfaSleepMilsec(1);
            sleepTotal++;
            continue;
        }
        ...
        if (extraTimeSpendOnSending > 0)
        {
            if ( extraTimeSpendOnSending > theProf->burstperiodicity*1000 - difftime )
            {/* reduce sleep time to catch up over all on time sending   */
                extraTimeSpendOnSending = (extraTimeSpendOnSending - (theProf->burstperiodicity*1000 - difftime));
                wfaSleepMilsec(1);
                sleepTotal = sleepTotal + (unsigned long long int) 1;
                continue;
            }
        ...
        if(theProf->burstperiodicity - difftime > 0)
        {/*  only sleep if there is extrac time with in the second did not spend on sending */
            wfaSleepMilsec(theProf->burstperiodicity - difftime);
            sleepTotal = sleepTotal + (unsigned long long int)(theProf->burstperiodicity - difftime);
        }
        
        if ( theProf->duration * 1000 < nDuration * theProf->burstperiodicity)
        {
            nOverSend++;
            /* This should not happen */
            if (nOverSend > 1000) {
                DPRINT_INFO(WFA_OUT, "nDuration: %d, Loop: %d\n", nDuration, runLoop);
                break;
            }
        }
    ...
    *pRespLen = WFA_TLV_HDR_LEN + sizeof(dutCmdResponse_t);
    ...
    extraTimeSpendOnSending = extraTimeSpendOnSending/1000;
    DPRINT_INFO(WFA_OUT, "*** wfg_tg.cpp wfaSendBurstData Count=%i txFrames=%i totalByteSent=%i sleepTotal=%llu milSec extraTimeSpendOnSending=%llu nOverTimeCount=%d nOverSend=%i BurstFrag=%d BurstPeriod=%d nDuration=%d ***\n", 
        counter, (myStream->stats.txFrames),(unsigned int) (myStream->stats.txPayloadBytes), sleepTotal,extraTimeSpendOnSending, nOverTimeCount, nOverSend, theProf->burstfrags, theProf->burstperiodicity, nDuration);
    wfaSleepMilsec(1000);
    return ret;
