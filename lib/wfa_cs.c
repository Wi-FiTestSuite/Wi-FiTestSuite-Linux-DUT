...
int wfaSanitizeShell(char *command, char *new)
{
    int replace = 0;
    memset(new, '\0', strlen(command) + 1);
    for(int i=0; i<strlen(command); i++){
        if(command[i] == '\''){
            replace--;
        }else{
            new[i+replace] = command[i];
        }
    }
    return replace;
}
...
int wfaStaAssociate(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char ifname_s[WFA_IF_NAME_LEN];
    wfaSanitizeShell(ifname, ifname_s);
    ...
    else
    {
      ...
      //sprintf(gCmdStr, "ifconfig '%s' down", ifname_s);
      //sret = system(gCmdStr);

        /* use 'ifconfig' command to bring up the interface (linux specific) */
      //sprintf(gCmdStr, "ifconfig '%s' up", ifname_s);
      //sret = system(gCmdStr);
         ...
         */
        sprintf(gCmdStr, "wpa_cli -i'%s' reassociate", ifname_s);
        sret = system(gCmdStr);
    }
...
int wfaStaIsConnected(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
     ...
     */
    sprintf(gCmdStr, "/sbin/wpa_cli -i'%s' status | grep ^wpa_state= | cut -f2- -d= > /tmp/.isConnected", ifname_s);
    sret = system(gCmdStr);
...
int wfaStaSetIpConfig(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char ifname_s[WFA_IF_NAME_LEN];
    char ipaddr_s[WFA_IP_ADDR_STR_LEN];
    char mask_s[WFA_IP_ADDR_STR_LEN];
    char defGateway_s[WFA_IP_ADDR_STR_LEN];
    char pri_dns_s[WFA_IP_ADDR_STR_LEN];
    char sec_dns_s[WFA_IP_ADDR_STR_LEN];
    wfaSanitizeShell(ipconfig->intf, ifname_s);
    wfaSanitizeShell(ipconfig->ipaddr, ipaddr_s);
    wfaSanitizeShell(ipconfig->mask, mask_s);
    wfaSanitizeShell(ipconfig->defGateway, defGateway_s);
    wfaSanitizeShell(ipconfig->pri_dns, pri_dns_s);
    wfaSanitizeShell(ipconfig->sec_dns, sec_dns_s);
...
int wfaStaVerifyIpConnection(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char dipaddr_s[WFA_IF_NAME_LEN];
    wfaSanitizeShell(verip->cmdsu.verifyIp.dipaddr, dipaddr_s);
    ...
    sprintf(gCmdStr, "ping '%s' -c 3 -W %u | grep loss | cut -f3 -d, 1>& /tmp/pingout.txt", dipaddr_s, verip->cmdsu.verifyIp.timeout);
    sret = system(gCmdStr);
...
int wfaStaGetMacAddress(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
     ...
     */
    sprintf(gCmdStr, "ifconfig '%s' > /tmp/ipconfig.txt ", ifname_s);
    sret = system(gCmdStr);
...
int wfaSetEncryption1(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char ifname_s[WFA_IF_NAME_LEN];
    char ssid_s[WFA_SSID_NAME_LEN];
    wfaSanitizeShell(setEncryp->intf, ifname_s);
    wfaSanitizeShell(setEncryp->ssid, ssid_s);

    /*
     * disable the network first
     */
    sprintf(gCmdStr, "wpa_cli -i '%s' disable_network 0", ifname_s);
    sret = system(gCmdStr);

    /*
     * set SSID
     */
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 ssid '\"%s\"'", ifname_s, ssid_s);
    sret = system(gCmdStr);

    /*
     * Tell the supplicant for infrastructure mode (1)
     */
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 mode 0", ifname_s);
    sret = system(gCmdStr);

    /*
     * set Key management to NONE (NO WPA) for plaintext or WEP
     */
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 key_mgmt NONE", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' enable_network 0", ifname_s);
    sret = system(gCmdStr);
...
int wfaSetEncryption(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    if(setEncryp->encpType == 1)
    {
        ...
        if(setEncryp->keys[i][0] != '\0')
        {
            sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 wep_tx_keyidx %i",
                    ifname_s, setEncryp->activeKeyIdx);
            sret = system(gCmdStr);
        }
    }
    else /* clearly remove the keys -- reported by p.schwann */
    {
        ...
        for(i = 0; i < 4; i++)
        {
            sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 wep_key%i \"\"", ifname_s, i);
            sret = system(gCmdStr);
        }
    }
...
int wfaStaSetEapTLS(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char ifname_s[WFA_IF_NAME_LEN];
    char ssid_s[WFA_SSID_NAME_LEN];
    char trustedRootCA_s[128];
    char clientCertificate_s[128];
    wfaSanitizeShell(ifname, ifname_s);
    wfaSanitizeShell(setTLS->ssid, ssid_s);
    wfaSanitizeShell(setTLS->trustedRootCA, trustedRootCA_s);
    wfaSanitizeShell(setTLS->clientCertificate, clientCertificate_s);
     ...
     */
#ifdef WFA_NEW_CLI_FORMAT
    sprintf(gCmdStr, "wfa_set_eaptls -i '%s' '%s' '%s' '%s'", ifname_s, ssid_s, trustedRootCA_s, clientCertificate_s);
    sret = system(gCmdStr);
#else
    ...
    {
        sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 key_mgmt WPA-EAP", ifname_s);
    }
    ...
    /* protocol WPA */
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 proto WPA", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 eap TLS", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 ca_cert '\"'%s'\"'", ifname_s, setTLS->trustedRootCA);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 identity '\"wifi-user@wifilabs.local\"'", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 private_key '\"%s/%s\"'", ifname_s, CERTIFICATES_PATH, clientCertificate_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 private_key_passwd '\"wifi\"'", ifname_s);
    sret = system(gCmdStr);
...
int wfaStaSetPSK(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    wfaSanitizeShell(setPSK->intf, ifname_s);
    wfaSanitizeShell(setPSK->ssid, ssid_s);
    wfaSanitizeShell(setPSK->passphrase, passphrase_s);
#ifdef WFA_NEW_CLI_FORMAT
    sprintf(gCmdStr, "wfa_set_psk '%s' '%s' '%s'", ifname_s, ssid_s, passphrase_s);
    sret = system(gCmdStr);
#else
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 ssid '\"%s\"'", ifname_s, ssid_s);
    sret = system(gCmdStr);
    ...
    if(strcasecmp(setPSK->keyMgmtType, "wpa2-sha256") == 0)
        sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 key_mgmt WPA2-SHA256", ifname_s);
    ...
    else if (strcasecmp(setPSK->keyMgmtType, "wpa2-wpa-psk") == 0)
    {
        ...
        sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 key_mgmt WPA-PSK", ifname_s);

    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 psk '\"%s\"'", ifname_s, passphrase_s);
    sret = system(gCmdStr);
...
int wfaStaGetInfo(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char ifname_s[WFA_IF_NAME_LEN];
    wfaSanitizeShell(getInfo->intf, ifname_s);

    /*
     * Normally this is called to retrieve the vendor information
     * from a interface, no implement yet
     */
    sprintf(infoResp.cmdru.info, "interface,'%s',vendor,XXX,cardtype,802.11a/b/g", ifname_s);
...
int wfaStaSetEapTTLS(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char username_s[32];
    char passwd_s[16];
    char trustedRootCA_s[32];
    wfaSanitizeShell(ifname, ifname_s);
    wfaSanitizeShell(setTTLS->ssid, ssid_s);
    wfaSanitizeShell(setTTLS->username, username_s);
    wfaSanitizeShell(setTTLS->passwd, passwd_s);
    wfaSanitizeShell(setTTLS->trustedRootCA, trustedRootCA_s);

#ifdef WFA_NEW_CLI_FORMAT
    sprintf(gCmdStr, "wfa_set_eapttls '%s' '%s' '%s' '%s' '%s'", ifname, ssid_s, username_s, setTTLS->passwd, setTTLS->trustedRootCA);
    sret = system(gCmdStr);
#else
    ...
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 identity '\"%s\"'", ifname_s, username_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 password '\"%s\"'", ifname_s, passwd_s);
    sret = system(gCmdStr);
...
//   sprintf(cmdStr, "wpa_cli -i '%s' set_network 0 pairwise '\"%s\"", ifname_s, setTTLS->encrptype);
    ...
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 eap TTLS", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 ca_cert '\"%s/%s\"'", ifname_s, CERTIFICATES_PATH, trustedRootCA_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 proto WPA", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 phase2 '\"auth=MSCHAPV2\"'", ifname_s);
    sret = system(gCmdStr);
...
int wfaStaSetEapSIM(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char encrptype_s[9];
    wfaSanitizeShell(ifname, ifname_s);
    wfaSanitizeShell(setSIM->ssid, ssid_s);
    wfaSanitizeShell(setSIM->username, username_s);
    wfaSanitizeShell(setSIM->encrptype, encrptype_s);

#ifdef WFA_NEW_CLI_FORMAT
    sprintf(gCmdStr, "wfa_set_eapsim '%s' '%s' '%s' '%s'", ifname_s, ssid_s, username_s, encrptype_s);
    sret = system(gCmdStr);
#else
    ...
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 pairwise '\"%s\"'", ifname_s, encrptype_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 eap SIM", ifname_s);
    sret = system(gCmdStr);
    ...
    {
        sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 key_mgmt WPA-SHA256", ifname_s);
    }
    ...
    {
        sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 key_mgmt WPA-FT", ifname_s);
    }
...
int wfaStaSetPEAP(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char innerEAP_s[16];
    wfaSanitizeShell(ifname, ifname_s);
    wfaSanitizeShell(setPEAP->ssid, ssid_s);
    wfaSanitizeShell(setPEAP->username, username_s);
    wfaSanitizeShell(setPEAP->passwd, passwd_s);
    wfaSanitizeShell(setPEAP->encrptype, encrptype_s);
    wfaSanitizeShell(setPEAP->trustedRootCA, trustedRootCA_s);
    wfaSanitizeShell(setPEAP->innerEAP, innerEAP_s);


#ifdef WFA_NEW_CLI_FORMAT
    sprintf(gCmdStr, "wfa_set_peap '%s' '%s' '%s' '%s' '%s' '%s' %i '%s'", ifname, ssid_s, username_s,
            passwd_s, setPEAP->trustedRootCA,
    ...
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 eap PEAP", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 anonymous_identity '\"anonymous\"' ", ifname_s);
    sret = system(gCmdStr);
    ...
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 ca_cert '\"'%s'/'%s'\"'", ifname_s, CERTIFICATES_PATH, setPEAP->trustedRootCA);
    sret = system(gCmdStr);
    ...
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 phase1 '\"peaplabel=%i\"'", ifname_s, setPEAP->peapVersion);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 phase2 '\"auth='%s'\"'", ifname_s, setPEAP->innerEAP);
    sret = system(gCmdStr);
...
int wfaStaSetIBSS(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char ifname_s[WFA_IF_NAME_LEN];
    char ssid_s[WFA_SSID_NAME_LEN];
    wfaSanitizeShell(setIBSS->intf, ifname_s);
    wfaSanitizeShell(setIBSS->ssid, ssid_s);
     ...
     */
    sprintf(gCmdStr, "iwconfig '%s' channel %i", ifname_s, setIBSS->channel);
    sret = system(gCmdStr);

    /*
     * Tell the supplicant for IBSS mode (1)
     */
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 mode 1", ifname_s);
    sret = system(gCmdStr);
...
int wfaStaSetMode(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    int i;
    char intf_s[WFA_IF_NAME_LEN];
    char ssid_s[WFA_SSID_NAME_LEN];
    wfaSanitizeShell(setmode->intf, intf_s);
    wfaSanitizeShell(setmode->ssid, ssid_s);

    /*
     * bring down the interface
     */
    sprintf(gCmdStr, "ifconfig '%s' down",intf_s);
    sret = system(gCmdStr);

    /*
     * distroy the interface
     */
    sprintf(gCmdStr, "wlanconfig '%s' destroy",intf_s);
    sret = system(gCmdStr);
     ...
     */
    if(setmode->mode == 1)
        sprintf(gCmdStr, "wlanconfig '%s' create wlandev wifi0 wlanmode adhoc",intf_s);
    else
        sprintf(gCmdStr, "wlanconfig '%s' create wlandev wifi0 wlanmode managed",intf_s);
        ...
        for(i=0; i<4; i++)
        {
            if(setmode->keys[i][0] != '\0')
            {
                sprintf(gCmdStr, "iwconfig  '%s' key  s:'%s'",
                        intf_s, setmode->keys[i]);
                sret = system(gCmdStr);
            }
            /* set active key */
            if(setmode->keys[j][0] != '\0')
                sprintf(gCmdStr, "iwconfig  '%s' key  s:'%s'",
                        intf_s, setmode->keys[j]);
            sret = system(gCmdStr);
        }
    ...
    if(setmode->channel)
    {
        sprintf(gCmdStr, "iwconfig '%s' channel %i", intf_s, setmode->channel);
        sret = system(gCmdStr);
    }
     ...
     * set SSID
     */
    sprintf(gCmdStr, "iwconfig '%s' essid '%s'", intf_s, ssid_s);
    sret = system(gCmdStr);

    /*
     * bring up the interface
     */
    sprintf(gCmdStr, "ifconfig '%s' up", intf_s);
    sret = system(gCmdStr);
...
int wfaStaSetPwrSave(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char intf_s[WFA_IF_NAME_LEN];
    char mode_s[8];
    wfaSanitizeShell(setps->intf, intf_s);
    wfaSanitizeShell(setps->mode, mode_s);



    sprintf(gCmdStr, "iwconfig '%s' power '%s'", intf_s, mode_s);
    sret = system(gCmdStr);
...
int wfaStaSetWMM(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    case GROUP_WMMCONF:
        sprintf(gCmdStr, "iwconfig '%s' rts %d",
                ifname_s,setwmm->actions.config.rts_thr);

        sret = system(gCmdStr);
        sprintf(gCmdStr, "iwconfig '%s' frag %d",
                ifname_s,setwmm->actions.config.frag_thr);

        sret = system(gCmdStr);
        sprintf(gCmdStr, "iwpriv '%s' wmmcfg %d",
                ifname_s, setwmm->actions.config.wmm);
...
int wfaStaSetEapFAST(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char passwd_s[16];
    char innerEAP_s[16];
    char pacFileName_s[32];
    wfaSanitizeShell(ifname, ifname_s);
    wfaSanitizeShell(setFAST->ssid, ssid_s);
    wfaSanitizeShell(setFAST->username, username_s);
    wfaSanitizeShell(setFAST->passwd, passwd_s);
    wfaSanitizeShell(setFAST->innerEAP, innerEAP_s);
    wfaSanitizeShell(setFAST->pacFileName, pacFileName_s);

#ifdef WFA_NEW_CLI_FORMAT
    sprintf(gCmdStr, "wfa_set_eapfast '%s' '%s' '%s' '%s' '%s' '%s'", ifname_s, ssid_s, username_s,
            passwd_s, pacFileName_s,
            setFAST->innerEAP);
    sret = system(gCmdStr);
#else
    ...
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 eap FAST", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 pac_file '\"%s/%s\"'", ifname_s, CERTIFICATES_PATH,     setFAST->pacFileName);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 anonymous_identity '\"anonymous\"'", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 phase1 '\"fast_provisioning=1\"'", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 phase2 '\"auth=%s\"'", ifname_s, innerEAP_s);
    sret = system(gCmdStr);
...
int wfaStaSetEapAKA(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char passwd_s[96];
    wfaSanitizeShell(ifname, ifname_s);
    wfaSanitizeShell(setAKA->ssid, ssid_s);
    wfaSanitizeShell(setAKA->username, username_s);
    wfaSanitizeShell(setAKA->passwd, passwd_s);

#ifdef WFA_NEW_CLI_FORMAT
    sprintf(gCmdStr, "wfa_set_eapaka '%s' '%s' '%s' '%s'", ifname_s, ssid_s, username_s, passwd_s);
    sret = system(gCmdStr);
#else
    ...
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 proto WPA2", ifname_s);
    sret = system(gCmdStr);
    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 proto CCMP", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 eap AKA", ifname_s);
    sret = system(gCmdStr);

    sprintf(gCmdStr, "wpa_cli -i '%s' set_network 0 phase1 \"result_ind=1\"", ifname_s);
    sret = system(gCmdStr);
...
int wfaStaResetDefault(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    char prog_s[8];
    char type_s[8];
    wfaSanitizeShell(reset->intf, ifname_s);
    wfaSanitizeShell(reset->prog, prog_s);
    wfaSanitizeShell(reset->type, type_s);


    // need to make your own command available for this, here is only an example
    sprintf(gCmdStr, "myresetdefault '%s' program '%s'", ifname_s, prog_s);
    sret = system(gCmdStr);
...
//    char *addr = staPing->dipaddr;
    ...
    char bflag[] = "  ";
#endif
    char dipaddr_s[IPV6_ADDRESS_STRING_LEN];
    wfaSanitizeShell(staPing->dipaddr, dipaddr_s);
    ...
    if (staPing->iptype == 2)
    {
        if ( tos>0)
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;wfaping6.sh %s '%s' -i %f -c %i -Q %d -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid,streamid,bflag, dipaddr_s, *interval, totalpkts, tos,  staPing->frameSize,streamid);
        else
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;wfaping6.sh %s '%s' -i %f -c %i -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid,streamid,bflag, dipaddr_s, *interval, totalpkts, staPing->frameSize,streamid);
        sret = system(cmdStr);
