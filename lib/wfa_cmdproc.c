...
typeNameStr_t keywordStr[] =
{
	...
	{ KW_HTI,          "hti",           NULL},
	{ KW_DSCP,         "dscp",          NULL},
	{ KW_BURSTPERIODICITY,"burstPeriodicity",  NULL},
	{ KW_BURSTFRAGS,   "burstFragments",    NULL},
};
        ...
        for(i = 0; i<sizeof(keywordStr)/sizeof(typeNameStr_t); i++)
        {
            if(strcasecmp(str, keywordStr[i].name) == 0)
            {
                ...
                case KW_DSCP:
                    ...
                    if( strcasecmp(str, "46") == 0 )
                    {
                        pf->trafficClass = TG_DSCP_46;
                    }
                    else if( strcasecmp(str, "34") == 0 )
                    {
                        pf->trafficClass = TG_DSCP_34;
                    }
                ...
                case KW_BURSTPERIODICITY:
                    str = strtok_r(NULL, ",", &pcmdStr);
                    if(isNumber(str) == WFA_FAILURE)
                    {
                        DPRINT_ERR(WFA_ERR, "Incorrect burst period format\n");
                        return WFA_FAILURE;
                    }
                    DPRINT_INFO(WFA_OUT, "burst period %s\n", str);
                    pf->burstperiodicity = atoi(str);
                    kwcnt++;
                    str = NULL;
                    break;
                ...
                case KW_BURSTFRAGS:
                    ...
                    if(isNumber(str) == WFA_FAILURE)
                    {
                        DPRINT_ERR(WFA_ERR, "Incorrect burst frags format\n");
                        return WFA_FAILURE;
                    }
                    DPRINT_INFO(WFA_OUT, "burst frags %s\n", str);
                    pf->burstfrags = atoi(str);
                    kwcnt++;
                    str = NULL;
                    break;
